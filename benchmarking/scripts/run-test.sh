...
if nc -z 127.0.0.1 30000; then
    python3 get-prometheus-timeseries-data.py http://127.0.0.1:30000 \
        -q "rate(container_cpu_usage_seconds_total{container=~\"nri-.*\"}[15s]),container_memory_usage_bytes{container=~\"nri-.*\"},container_memory_working_set_bytes{container=~\"nri-.*\"}" \
...
echo Executing: python3 get-jaeger-tracing-data.py http://127.0.0.1:30001 -c "${BASE_DIR}/output/${OUTPUT_FILE_PREFIX}-jaeger.csv" -s "${START_TIME}" -e "${END_TIME}" -r "$RUNTIME"

python3 get-jaeger-tracing-data.py http://127.0.0.1:30001 -c "${BASE_DIR}/output/${OUTPUT_FILE_PREFIX}-jaeger.csv" -s "${START_TIME}" -e "${END_TIME}" -r "$RUNTIME"
